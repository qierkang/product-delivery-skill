...
```text
ac06bb3 (origin/master) Merge branch 'master' of internal remote  ← 合并远程 Initial commit
...
远程在仓库创建时自动塞了一个 `Initial commit`，含一行 `README.md`。
...
- `dec-0004` accepted
- `CHANGELOG.md` v0.2.0 顶部已记录"首次独立入库"

## 后续动作建议

- [ ] 在托管平台配置 default branch protection（禁 force push 到 master）
